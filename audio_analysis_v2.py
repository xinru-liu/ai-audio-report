...
def generate_key_takeaways(transcript, summaries, topics, num_takeaways=5, use_gpu=True):
    """
    Generate insightful key takeaways from the transcript using a multi-stage analysis approach.
    
    This improved function uses:
    ...
    """
    print("Generating key takeaways with deep analysis...")
    ...
    try:
        ...
        # Load the model - Flan-T5 works well for analytical reasoning
        ...
        topic_takeaways = analyze_topics(
        ...
        if len(topics) > 1:
            relationship_takeaways = analyze_relationships(
                transcript, summaries, topics, segment_importances,
                tokenizer, model, device
            )
        ...
        implication_takeaways = analyze_implications(
            transcript, summaries, topics, segment_importances,
            tokenizer, model, device
        )
        ...
        print(f"Error in advanced takeaway generation: {e}")
...
def analyze_topics(transcript, summaries, topics, importances, tokenizer, model, device):
    """
    Analyze individual topics in depth.
    ...
    for topic_name, topic_score in top_topics:
        ...
        # Create a specific prompt for deeper analysis of this topic
        prompt = f"""
        Analyze the following meeting content about "{topic_name}" (relevance score: {topic_score:.2f}).
        
        MEETING CONTEXT:
        {relevant_context}
        
        Based on this context, provide 1-2 insightful takeaways specifically about "{topic_name}".
        ...
        outputs = model.generate(
            inputs.input_ids,
            max_length=300,  # Longer to allow for depth
            min_length=100,  # Ensure substantial content
...
def analyze_relationships(transcript, summaries, topics, importances, tokenizer, model, device):
    """
    Analyze relationships between topics and identify cross-cutting insights.
    ...
    prompt = f"""
    The following topics were discussed in a meeting:
    {", ".join(topic_names)}
...
def analyze_implications(transcript, summaries, topics, importances, tokenizer, model, device):
    """
    Analyze broader implications and high-level insights.
    ...
    # Create a prompt for implications analysis
    prompt = f"""
...
def fallback_takeaway_generation(transcript, summaries, topics, num_takeaways=5, use_gpu=True):
    ...
    try:
        ...
        # Simplified approach - break content into chunks and analyze each
        chunks = []
        chunk_size = max(1, len(summaries) // 3)  # Divide into ~3 chunks
        
        for i in range(0, len(summaries), chunk_size):
            chunk = " ".join(summaries[i:i+chunk_size])
            chunks.append(chunk)
        
        # Extract topic names
        topic_names = [t[0] for t in topics[:5]]
        ...
        for i, chunk in enumerate(chunks):
            try:
                # Load model for each chunk to avoid memory issues
                model_name = "google/flan-t5-large"
                tokenizer = AutoTokenizer.from_pretrained(model_name)
                model = AutoModelForSeq2SeqLM.from_pretrained(model_name).to(device)
                ...
                # Simplified prompt
                prompt = f"""
                Analyze this portion of a meeting about {topic_str}:
                
                {chunk[:2000]}
                
                What are 2-3 key insights or takeaways from this content?
                Be specific and insightful, focusing on the most important points.
                """
                ...
                # Extract potential takeaways by splitting on obvious markers
                lines = [line.strip() for line in result.split('\n') if line.strip()]
                for line in lines:
                    # Look for numbered points, bullet points, or just substantial sentences
                    if re.match(r'^\d+\.', line) or line.startswith('-') or line.startswith('*'):
                        takeaway = re.sub(r'^\d+\.\s*|-\s*|\*\s*', '', line).strip()
                        all_takeaways.append(takeaway)
                    elif len(line.split()) >= 8:
                        all_takeaways.append(line)
        ...
        # If we still don't have enough, extract important sentences directly
        if len(all_takeaways) < num_takeaways:
            try:
                ...
                for sentence, _ in sentence_scores[:num_takeaways - len(all_takeaways)]:
                    all_takeaways.append(sentence)
            except Exception as e:
        ...
        for takeaway in all_takeaways:
            # Skip very short takeaways
            if len(takeaway.split()) < 5:
                continue
                
            # Skip duplicates
            takeaway_lower = takeaway.lower()
            if takeaway_lower in seen:
                continue
                
            seen.add(takeaway_lower)
            
            # Ensure proper capitalization and punctuation
            formatted = takeaway[0].upper() + takeaway[1:]
